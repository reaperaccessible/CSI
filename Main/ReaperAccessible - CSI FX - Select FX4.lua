...
-- 2) Vérifier présence d’un FX en slot 4 (index 3)
local fxcount = reaper.TrackFX_GetCount(track)
if fxcount < 4 then
  speak("No FX in slot 4")
  return
end

-- 3) Tenter SWS _S&M_SELFX4, sinon TrackFX_Show(track, 3, 1)
local cmd = reaper.NamedCommandLookup("_S&M_SELFX4")
if cmd ~= 0 then
  reaper.Main_OnCommand(cmd, 0)
else
  reaper.TrackFX_Show(track, 3, 1) -- 1 = show and focus
end

-- 4) Cibler slot 4 (index 3)
local fx_index = 3
...
reaper.PreventUIRefresh(-1)
reaper.Undo_EndBlock2(0, "Toggle FX slot 4; Amp policy and announce", -1)
