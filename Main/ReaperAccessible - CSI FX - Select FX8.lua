...
-- 2) Vérifier présence d’un FX en slot 8 (index 7)
local fxcount = reaper.TrackFX_GetCount(track)
if fxcount < 8 then
  speak("No FX in slot 8")
  return
end

-- 3) Tenter SWS _S&M_SELFX8, sinon TrackFX_Show(track, 7, 1)
local cmd = reaper.NamedCommandLookup("_S&M_SELFX8")
if cmd ~= 0 then
  reaper.Main_OnCommand(cmd, 0)
else
  reaper.TrackFX_Show(track, 7, 1) -- 1 = show and focus
end

-- 4) Cibler slot 8 (index 7)
local fx_index = 7
...
reaper.PreventUIRefresh(-1)
reaper.Undo_EndBlock2(0, "Toggle FX slot 8; Amp policy and announce", -1)
