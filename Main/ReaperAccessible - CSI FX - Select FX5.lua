...
-- 2) Vérifier présence d’un FX en slot 5 (index 4)
local fxcount = reaper.TrackFX_GetCount(track)
if fxcount < 5 then
  speak("No FX in slot 5")
  return
end

-- 3) Tenter SWS _S&M_SELFX5, sinon TrackFX_Show(track, 4, 1)
local cmd = reaper.NamedCommandLookup("_S&M_SELFX5")
if cmd ~= 0 then
  reaper.Main_OnCommand(cmd, 0)
else
  reaper.TrackFX_Show(track, 4, 1) -- 1 = show and focus
end

-- 4) Cibler slot 5 (index 4)
local fx_index = 4
...
reaper.PreventUIRefresh(-1)
reaper.Undo_EndBlock2(0, "Toggle FX slot 5; Amp policy and announce", -1)
