...
local function is_amp_by_base(name)
  -- Sensible à la casse. On exige "Amp" avant le premier tiret.
...
-- 2) Vérifier présence d’un FX en slot 2 (index 1)
local fxcount = reaper.TrackFX_GetCount(track)
if fxcount < 2 then
  speak("No FX in slot 2")
  return
end

-- 3) Tenter SWS _S&M_SELFX2, sinon TrackFX_Show(track, 1, 1)
local cmd = reaper.NamedCommandLookup("_S&M_SELFX2")
if cmd ~= 0 then
  reaper.Main_OnCommand(cmd, 0)
else
  reaper.TrackFX_Show(track, 1, 1) -- 1 = show and focus
end

-- 4) Cibler slot 2 (index 1)
local fx_index = 1
...
-- Si c’est un Amp, bypass tous les autres Amp (détection avant tiret, case-sensitive)
...
reaper.PreventUIRefresh(-1)
reaper.Undo_EndBlock2(0, "Toggle FX slot 2; Amp policy and announce", -1)
